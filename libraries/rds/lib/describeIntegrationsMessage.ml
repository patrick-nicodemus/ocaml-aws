...
type t =
  { integration_identifier : String.t option
  ; filters : FilterList.t
  ; max_records : Integer.t option
  ; marker : String.t option
  }

let make ?integration_identifier ?(filters = []) ?max_records ?marker () =
  { integration_identifier; filters; max_records; marker }
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.integration_identifier (fun f ->
             Aws.Query.Pair ("IntegrationIdentifier", String.to_query f))
       ])
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.integration_identifier (fun f ->
             "IntegrationIdentifier", String.to_json f)
       ])

let of_json j =
  { integration_identifier =
      Aws.Util.option_map (Aws.Json.lookup j "IntegrationIdentifier") String.of_json
