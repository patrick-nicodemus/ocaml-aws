...
let make ~amazon_side_asn  ?client_token  ?dry_run  ?persist_routes 
  ?persist_routes_duration  ?sns_notifications_enabled  ?(tag_specifications=
  [])  () =
  {
    amazon_side_asn;
    client_token;
...
let parse xml =
  Some
    {
      amazon_side_asn =
        (Aws.Xml.required "AmazonSideAsn"
           (Aws.Util.option_bind (Aws.Xml.member "AmazonSideAsn" xml)
              Long.parse));
      client_token =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair ("AmazonSideAsn", (Long.to_query v.amazon_side_asn)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Some
...
let of_json j =
  {
    amazon_side_asn =
      (Long.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "AmazonSideAsn")));
    client_token =
