  ...
  Some
    {
      connection_notification =
        (Aws.Util.option_bind (Aws.Xml.member "connectionNotification" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.connection_notification
         (fun f ->
            Aws.Query.Pair
              ("ConnectionNotification", (ConnectionNotification.to_query f)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.client_token
...
let of_json j =
  {
    connection_notification =
      (Aws.Util.option_map (Aws.Json.lookup j "connectionNotification")
