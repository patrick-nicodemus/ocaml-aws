...
let make ?capacity_reservation_id  ?capacity_block_id  ?state  ?instance_type
   ?group_name  ?(network_nodes= [])  ?availability_zone_id 
  ?availability_zone  () =
  {
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("NetworkNodeSet", (NetworkNodeSet.to_query v.network_nodes)));
