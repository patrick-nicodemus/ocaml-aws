...
let make ?secondary_network ?client_token () = { secondary_network; client_token }

let parse xml =
  Some
    { secondary_network =
        Aws.Util.option_bind
          (Aws.Xml.member "secondaryNetwork" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.secondary_network (fun f ->
             Aws.Query.Pair ("SecondaryNetwork", SecondaryNetwork.to_query f))
       ])
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.secondary_network (fun f ->
             "secondaryNetwork", SecondaryNetwork.to_json f)
       ])

let of_json j =
  { secondary_network =
      Aws.Util.option_map (Aws.Json.lookup j "secondaryNetwork") SecondaryNetwork.of_json
