  ...
  Some
    {
      ...
      instance_tags =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "InstanceTag" xml)
              TagList.parse));
      dedicated_host_ids =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "DedicatedHostId" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair ("InstanceTag", (TagList.to_query v.instance_tags)));
       Some
         (Aws.Query.Pair
            ("InstanceId", (InstanceIdList.to_query v.instance_ids)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Some
...
let of_json j =
  {
    ...
    instance_tags =
      (TagList.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "InstanceTag")));
    dedicated_host_ids =
