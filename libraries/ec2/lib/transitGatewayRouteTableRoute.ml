...
let make ?destination_cidr  ?state  ?route_origin  ?prefix_list_id 
  ?attachment_id  ?resource_id  ?resource_type  () =
  {
    destination_cidr;
    state;
    route_origin;
  ...
  Some
    {
      ...
      route_origin =
        (Aws.Util.option_bind (Aws.Xml.member "routeOrigin" xml) String.parse);
      prefix_list_id =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.route_origin
         (fun f -> Aws.Query.Pair ("RouteOrigin", (String.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.route_origin
         (fun f -> ("routeOrigin", (String.to_json f)));
...
let of_json j =
  {
    ...
    route_origin =
      (Aws.Util.option_map (Aws.Json.lookup j "routeOrigin") String.of_json);
    prefix_list_id =
