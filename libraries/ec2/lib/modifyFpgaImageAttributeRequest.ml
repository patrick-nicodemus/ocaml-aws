...
let make ?dry_run  ~fpga_image_id  ?attribute  ?operation_type  ?(user_ids=
  [])  ?(user_groups= [])  ?(product_codes= [])  ?load_permission 
  ?description  ?name  () =
  {
    dry_run;
    fpga_image_id;
  ...
  Some
    {
      ...
      load_permission =
        (Aws.Util.option_bind (Aws.Xml.member "LoadPermission" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.load_permission
         (fun f ->
            Aws.Query.Pair
              ("LoadPermission", (LoadPermissionModifications.to_query f)));
       Some
         (Aws.Query.Pair
...
let of_json j =
  {
    ...
    load_permission =
      (Aws.Util.option_map (Aws.Json.lookup j "LoadPermission")
