...
let make ?dry_run  ?(filters= [])  ?(exclusion_ids= [])  ?next_token 
  ?max_results  () =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("ExclusionId",
              (VpcBlockPublicAccessExclusionIdList.to_query v.exclusion_ids)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Some
         ("ExclusionId",
           (VpcBlockPublicAccessExclusionIdList.to_json v.exclusion_ids));
