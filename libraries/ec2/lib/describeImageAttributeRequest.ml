...
let parse xml =
  Some
    { attribute =
        Aws.Xml.required
          "Attribute"
          (Aws.Util.option_bind (Aws.Xml.member "Attribute" xml) ImageAttributeName.parse)
    ; image_id =
        Aws.Xml.required
          "ImageId"
...
let of_json j =
  { attribute =
      ImageAttributeName.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "Attribute"))
