  ...
  Some
    {
      service_names =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "serviceNameSet" xml)
              ValueStringList.parse));
      service_details =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "serviceDetailSet" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("ServiceNameSet", (ValueStringList.to_query v.service_names)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.next_token
       ...
       Some
         ("serviceDetailSet", (ServiceDetailSet.to_json v.service_details));
