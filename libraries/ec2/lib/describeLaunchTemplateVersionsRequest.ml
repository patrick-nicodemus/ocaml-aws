...
let make ?dry_run  ?launch_template_id  ?launch_template_name  ?(versions=
  [])  ?min_version  ?max_version  ?next_token  ?max_results  ?(filters= []) 
  ?resolve_alias  ?include_managed_resources  () =
  {
    dry_run;
    launch_template_id;
  ...
  Some
    {
      ...
      min_version =
        (Aws.Util.option_bind (Aws.Xml.member "MinVersion" xml) String.parse);
      max_version =
        (Aws.Util.option_bind (Aws.Xml.member "MaxVersion" xml) String.parse);
      next_token =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.max_version
         (fun f -> Aws.Query.Pair ("MaxVersion", (String.to_query f)));
       Aws.Util.option_map v.min_version
         (fun f -> Aws.Query.Pair ("MinVersion", (String.to_query f)));
       Some
         (Aws.Query.Pair
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.max_version
         (fun f -> ("MaxVersion", (String.to_json f)));
       Aws.Util.option_map v.min_version
         (fun f -> ("MinVersion", (String.to_json f)));
...
let of_json j =
  {
    ...
    min_version =
      (Aws.Util.option_map (Aws.Json.lookup j "MinVersion") String.of_json);
    max_version =
      (Aws.Util.option_map (Aws.Json.lookup j "MaxVersion") String.of_json);
    next_token =
