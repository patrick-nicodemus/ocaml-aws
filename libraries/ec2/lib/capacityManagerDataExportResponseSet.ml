...
let parse xml =
  Aws.Util.option_all
    (List.map CapacityManagerDataExportResponse.parse (Aws.Xml.members "item" xml))

let to_query v = Aws.Query.to_query_list CapacityManagerDataExportResponse.to_query v
