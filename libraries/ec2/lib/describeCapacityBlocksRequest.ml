...
let make ?(capacity_block_ids= [])  ?next_token  ?max_results  ?(filters= [])
   ?dry_run  () =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("CapacityBlockId",
              (CapacityBlockIds.to_query v.capacity_block_ids)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.dry_run
       ...
       Some
         ("CapacityBlockId", (CapacityBlockIds.to_json v.capacity_block_ids))])
let of_json j =
  {
    capacity_block_ids =
