...
let make ~verified_access_trust_provider_id  ?oidc_options  ?device_options 
  ?description  ?dry_run  ?client_token  ?sse_specification 
  ?native_application_oidc_options  () =
  {
...
let parse xml =
  Some
    {
      verified_access_trust_provider_id =
        (Aws.Xml.required "VerifiedAccessTrustProviderId"
    ...
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.device_options
         (fun f ->
            Aws.Query.Pair
              ("DeviceOptions",
                (ModifyVerifiedAccessTrustProviderDeviceOptions.to_query f)));
       Aws.Util.option_map v.oidc_options
         (fun f ->
            Aws.Query.Pair
              ("OidcOptions",
                (ModifyVerifiedAccessTrustProviderOidcOptions.to_query f)));
       Some
         (Aws.Query.Pair
  ...
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.device_options
         (fun f ->
            ("DeviceOptions",
              (ModifyVerifiedAccessTrustProviderDeviceOptions.to_json f)));
       Aws.Util.option_map v.oidc_options
         (fun f ->
            ("OidcOptions",
              (ModifyVerifiedAccessTrustProviderOidcOptions.to_json f)));
       Some
         ("VerifiedAccessTrustProviderId",
