...
let parse xml =
  Some
    {
      resource =
        (Aws.Xml.required "Resource"
           (Aws.Util.option_bind (Aws.Xml.member "Resource" xml) String.parse));
      use_long_ids =
        (Aws.Xml.required "UseLongIds"
           (Aws.Util.option_bind (Aws.Xml.member "UseLongIds" xml)
              Boolean.parse))
    }
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Some
          (Aws.Query.Pair ("UseLongIds", (Boolean.to_query v.use_long_ids)));
...
let of_json j =
  {
    resource =
      (String.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "Resource")));
    use_long_ids =
      (Boolean.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "UseLongIds")))
  }