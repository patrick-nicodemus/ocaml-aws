...
type t =
  ...
  ; memory_info : InferenceDeviceMemoryInfo.t option
  }
  ...
  Some
    ...
    ; memory_info =
        Aws.Util.option_bind
          ...
          InferenceDeviceMemoryInfo.parse
    }
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [ Aws.Util.option_map v.memory_info (fun f ->
             Aws.Query.Pair ("MemoryInfo", InferenceDeviceMemoryInfo.to_query f))
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [ Aws.Util.option_map v.memory_info (fun f ->
             "memoryInfo", InferenceDeviceMemoryInfo.to_json f)
...
let of_json j =
  ...
  ; memory_info =
      Aws.Util.option_map
        (Aws.Json.lookup j "memoryInfo")
        InferenceDeviceMemoryInfo.of_json
  }
