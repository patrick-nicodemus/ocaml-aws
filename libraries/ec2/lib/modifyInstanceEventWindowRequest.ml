...
let make ?dry_run  ?name  ~instance_event_window_id  ?(time_ranges= []) 
  ?cron_expression  () =
  ...
  Some
    {
      ...
      time_ranges =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "TimeRange" xml)
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("TimeRange",
              (InstanceEventWindowTimeRangeRequestSet.to_query v.time_ranges)));
       Some
         (Aws.Query.Pair
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Some
         ("TimeRange",
           (InstanceEventWindowTimeRangeRequestSet.to_json v.time_ranges));
       Some
         ("InstanceEventWindowId",
