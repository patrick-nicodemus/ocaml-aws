...
let make ?(subnet_ids= [])  ?port  ?rds_endpoint  () =
  { subnet_ids; port; rds_endpoint }
let parse xml =
  Some
    {
      subnet_ids =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("SubnetId",
              (ModifyVerifiedAccessEndpointSubnetIdList.to_query v.subnet_ids)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.rds_endpoint
       ...
       Some
         ("SubnetId",
           (ModifyVerifiedAccessEndpointSubnetIdList.to_json v.subnet_ids))])
let of_json j =
  {
    subnet_ids =
