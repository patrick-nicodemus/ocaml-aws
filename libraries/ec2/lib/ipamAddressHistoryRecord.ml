...
let make ?resource_owner_id  ?resource_region  ?resource_type  ?resource_id 
  ?resource_cidr  ?resource_name  ?resource_compliance_status 
  ?resource_overlap_status  ?vpc_id  ?sampled_start_time  ?sampled_end_time 
  () =
  {
    resource_owner_id;
    resource_region;
  ...
  Some
    {
      ...
      resource_name =
        (Aws.Util.option_bind (Aws.Xml.member "resourceName" xml)
           String.parse);
      resource_compliance_status =
        (Aws.Util.option_bind (Aws.Xml.member "resourceComplianceStatus" xml)
           IpamComplianceStatus.parse);
      resource_overlap_status =
        (Aws.Util.option_bind (Aws.Xml.member "resourceOverlapStatus" xml)
           IpamOverlapStatus.parse);
      vpc_id =
        (Aws.Util.option_bind (Aws.Xml.member "vpcId" xml) String.parse);
      sampled_start_time =
        (Aws.Util.option_bind (Aws.Xml.member "sampledStartTime" xml)
           DateTime.parse);
      sampled_end_time =
        (Aws.Util.option_bind (Aws.Xml.member "sampledEndTime" xml)
           DateTime.parse)
    }
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.sampled_end_time
          (fun f -> Aws.Query.Pair ("SampledEndTime", (DateTime.to_query f)));
       Aws.Util.option_map v.sampled_start_time
         (fun f -> Aws.Query.Pair ("SampledStartTime", (DateTime.to_query f)));
       Aws.Util.option_map v.vpc_id
         (fun f -> Aws.Query.Pair ("VpcId", (String.to_query f)));
       Aws.Util.option_map v.resource_overlap_status
         (fun f ->
            Aws.Query.Pair
              ("ResourceOverlapStatus", (IpamOverlapStatus.to_query f)));
       Aws.Util.option_map v.resource_compliance_status
         (fun f ->
            Aws.Query.Pair
              ("ResourceComplianceStatus", (IpamComplianceStatus.to_query f)));
       ...
       Aws.Util.option_map v.resource_type
         (fun f ->
            Aws.Query.Pair
              ("ResourceType", (IpamAddressHistoryResourceType.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.sampled_end_time
          (fun f -> ("sampledEndTime", (DateTime.to_json f)));
       Aws.Util.option_map v.sampled_start_time
         (fun f -> ("sampledStartTime", (DateTime.to_json f)));
       Aws.Util.option_map v.vpc_id (fun f -> ("vpcId", (String.to_json f)));
       Aws.Util.option_map v.resource_overlap_status
         (fun f -> ("resourceOverlapStatus", (IpamOverlapStatus.to_json f)));
       Aws.Util.option_map v.resource_compliance_status
         (fun f ->
...
let of_json j =
  {
    ...
    resource_compliance_status =
      (Aws.Util.option_map (Aws.Json.lookup j "resourceComplianceStatus")
         IpamComplianceStatus.of_json);
    resource_overlap_status =
      (Aws.Util.option_map (Aws.Json.lookup j "resourceOverlapStatus")
         IpamOverlapStatus.of_json);
    vpc_id = (Aws.Util.option_map (Aws.Json.lookup j "vpcId") String.of_json);
    sampled_start_time =
      (Aws.Util.option_map (Aws.Json.lookup j "sampledStartTime")
         DateTime.of_json);
    sampled_end_time =
      (Aws.Util.option_map (Aws.Json.lookup j "sampledEndTime")
         DateTime.of_json)
  }