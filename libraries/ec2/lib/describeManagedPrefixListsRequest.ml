...
let parse xml =
  Some
    ...
    ; prefix_list_ids =
        Aws.Util.of_option
          []
          (Aws.Util.option_bind (Aws.Xml.member "PrefixListId" xml) ValueStringList.parse)
    }
...
let of_json j =
  ...
  ; prefix_list_ids =
      ValueStringList.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "PrefixListId"))
  }
