...
let make ?next_token ?max_results ?dry_run ?(internet_gateway_ids = []) ?(filters = []) ()
    =
  { next_token; max_results; dry_run; internet_gateway_ids; filters }
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       ; Some
           (Aws.Query.Pair
              ("InternetGatewayId", InternetGatewayIdList.to_query v.internet_gateway_ids))
