type t = DeleteLaunchTemplateVersionsResponseSuccessItem.t list

let make elems () = elems

let parse xml =
  Aws.Util.option_all
    (List.map
       DeleteLaunchTemplateVersionsResponseSuccessItem.parse
       (Aws.Xml.members "item" xml))

let to_query v =
  Aws.Query.to_query_list DeleteLaunchTemplateVersionsResponseSuccessItem.to_query v

let to_json v = `List (List.map DeleteLaunchTemplateVersionsResponseSuccessItem.to_json v)
let of_json j = Aws.Json.to_list DeleteLaunchTemplateVersionsResponseSuccessItem.of_json j
