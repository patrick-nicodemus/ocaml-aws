...
let make ?(outpost_lag_ids= [])  ?(filters= [])  ?max_results  ?next_token 
  ?dry_run  () =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("OutpostLagId", (OutpostLagIdSet.to_query v.outpost_lag_ids)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.dry_run
