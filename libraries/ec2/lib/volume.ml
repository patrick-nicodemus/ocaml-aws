...
let make ?availability_zone_id  ?outpost_arn  ?source_volume_id  ?iops 
  ?(tags= [])  ~volume_type  ?fast_restored  ?multi_attach_enabled 
  ?throughput  ?sse_type  ?operator  ?volume_initialization_rate  ~volume_id 
  ~size  ~snapshot_id  ~availability_zone  ~state  ~create_time 
  ?(attachments= [])  ~encrypted  ?kms_key_id  () =
  {
  ...
  Some
    {
      ...
      volume_type =
        (Aws.Xml.required "volumeType"
           (Aws.Util.option_bind (Aws.Xml.member "volumeType" xml)
              VolumeType.parse));
      fast_restored =
        (Aws.Util.option_bind (Aws.Xml.member "fastRestored" xml)
           Boolean.parse);
      multi_attach_enabled =
      ...
      state =
        (Aws.Xml.required "status"
           (Aws.Util.option_bind (Aws.Xml.member "status" xml)
              VolumeState.parse));
      create_time =
        (Aws.Xml.required "createTime"
           (Aws.Util.option_bind (Aws.Xml.member "createTime" xml)
              DateTime.parse));
      attachments =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "attachmentSet" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("AttachmentSet", (VolumeAttachmentList.to_query v.attachments)));
       Some
         (Aws.Query.Pair ("CreateTime", (DateTime.to_query v.create_time)));
       ...
       Aws.Util.option_map v.fast_restored
         (fun f -> Aws.Query.Pair ("FastRestored", (Boolean.to_query f)));
       Some
         (Aws.Query.Pair ("VolumeType", (VolumeType.to_query v.volume_type)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.fast_restored
         (fun f -> ("fastRestored", (Boolean.to_json f)));
...
let of_json j =
  {
    ...
    volume_type =
      (VolumeType.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "volumeType")));
    fast_restored =
    ...
    size =
      (Integer.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "size")));
    snapshot_id =
      (String.of_json
    ...
    create_time =
      (DateTime.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "createTime")));
    attachments =
