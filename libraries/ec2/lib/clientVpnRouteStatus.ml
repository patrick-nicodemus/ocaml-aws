...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.code (fun f -> "code", ClientVpnRouteStatusCode.to_json f)
       ])

let of_json j =
  { code = Aws.Util.option_map (Aws.Json.lookup j "code") ClientVpnRouteStatusCode.of_json
