...
type t =
  ...
  external_authority_configuration:
    IpamScopeExternalAuthorityConfiguration.t option }
let make ?owner_id  ?ipam_scope_id  ?ipam_scope_arn  ?ipam_arn  ?ipam_region 
  ?ipam_scope_type  ?is_default  ?description  ?pool_count  ?state  ?(tags=
  [])  ?external_authority_configuration  () =
  {
    owner_id;
    ipam_scope_id;
  ...
  Some
    {
      ...
      pool_count =
        (Aws.Util.option_bind (Aws.Xml.member "poolCount" xml) Integer.parse);
      state =
        (Aws.Util.option_bind (Aws.Xml.member "state" xml)
           IpamScopeState.parse);
      tags =
        (Aws.Util.of_option []
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.external_authority_configuration
          (fun f ->
             Aws.Query.Pair
       ...
       Aws.Util.option_map v.state
         (fun f -> Aws.Query.Pair ("State", (IpamScopeState.to_query f)));
       Aws.Util.option_map v.pool_count
         (fun f -> Aws.Query.Pair ("PoolCount", (Integer.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.state
         (fun f -> ("state", (IpamScopeState.to_json f)));
       Aws.Util.option_map v.pool_count
         (fun f -> ("poolCount", (Integer.to_json f)));
       Aws.Util.option_map v.description
...
let of_json j =
  {
    ...
    pool_count =
      (Aws.Util.option_map (Aws.Json.lookup j "poolCount") Integer.of_json);
    state =
      (Aws.Util.option_map (Aws.Json.lookup j "state") IpamScopeState.of_json);
    tags =
