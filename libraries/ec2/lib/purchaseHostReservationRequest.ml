  ...
  Some
    {
      ...
      limit_price =
        (Aws.Util.option_bind (Aws.Xml.member "LimitPrice" xml) String.parse);
      offering_id =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.limit_price
         (fun f -> Aws.Query.Pair ("LimitPrice", (String.to_query f)));
       Some
         (Aws.Query.Pair
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.limit_price
         (fun f -> ("LimitPrice", (String.to_json f)));
...
let of_json j =
  {
    ...
    limit_price =
      (Aws.Util.option_map (Aws.Json.lookup j "LimitPrice") String.of_json);
    offering_id =
      (String.of_json
