...
let parse xml =
  Some
    { code =
        Aws.Util.option_bind (Aws.Xml.member "code" xml) IpamPoolCidrFailureCode.parse
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.code (fun f ->
             Aws.Query.Pair ("Code", IpamPoolCidrFailureCode.to_query f))
       ])
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.code (fun f -> "code", IpamPoolCidrFailureCode.to_json f)
       ])

let of_json j =
  { code = Aws.Util.option_map (Aws.Json.lookup j "code") IpamPoolCidrFailureCode.of_json
