...
let parse xml =
  Some
    {
      ebs =
        (Aws.Util.option_bind (Aws.Xml.member "ebs" xml) EbsBlockDevice.parse);
      no_device =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.ebs
         (fun f -> Aws.Query.Pair ("Ebs", (EbsBlockDevice.to_query f)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.virtual_name
...
let of_json j =
  {
    ebs =
      (Aws.Util.option_map (Aws.Json.lookup j "ebs") EbsBlockDevice.of_json);
    no_device =
