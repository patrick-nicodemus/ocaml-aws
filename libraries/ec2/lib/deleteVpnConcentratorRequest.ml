...
let make ~vpn_concentrator_id  ?dry_run  () =
  { vpn_concentrator_id; dry_run }
let parse xml =
  Some
    {
      vpn_concentrator_id =
        (Aws.Xml.required "VpnConcentratorId"
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("VpnConcentratorId", (String.to_query v.vpn_concentrator_id)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.dry_run
