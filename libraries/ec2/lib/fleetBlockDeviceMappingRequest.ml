...
type t =
  ...
  ; ebs : FleetEbsBlockDeviceRequest.t option
...
let parse xml =
  Some
    ...
    ; ebs =
        Aws.Util.option_bind (Aws.Xml.member "Ebs" xml) FleetEbsBlockDeviceRequest.parse
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.ebs (fun f ->
             Aws.Query.Pair ("Ebs", FleetEbsBlockDeviceRequest.to_query f))
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.ebs (fun f -> "Ebs", FleetEbsBlockDeviceRequest.to_json f)
...
let of_json j =
  ...
  ; ebs = Aws.Util.option_map (Aws.Json.lookup j "Ebs") FleetEbsBlockDeviceRequest.of_json
