...
let make ?network_insights_access_scope_id  ?(match_paths= []) 
  ?(exclude_paths= [])  () =
  ...
  Some
    {
      ...
      match_paths =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "matchPathSet" xml)
              AccessScopePathList.parse));
      exclude_paths =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "excludePathSet" xml)
              AccessScopePathList.parse))
    }
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Some
          (Aws.Query.Pair
             ("ExcludePathSet",
               (AccessScopePathList.to_query v.exclude_paths)));
       Some
         (Aws.Query.Pair
            ("MatchPathSet", (AccessScopePathList.to_query v.match_paths)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Some
          ("excludePathSet", (AccessScopePathList.to_json v.exclude_paths));
