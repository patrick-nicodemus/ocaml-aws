      ...
      fleet_ids =
        (Aws.Xml.required "FleetId"
           (Aws.Util.option_bind (Aws.Xml.member "FleetId" xml)
              FleetIdSet.parse));
      terminate_instances =
        (Aws.Xml.required "TerminateInstances"
           (Aws.Util.option_bind (Aws.Xml.member "TerminateInstances" xml)
              Boolean.parse))
    }
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Some
          (Aws.Query.Pair
             ("TerminateInstances", (Boolean.to_query v.terminate_instances)));
...
let of_json j =
  {
    ...
    terminate_instances =
      (Boolean.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "TerminateInstances")))
  }