      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (GetAllowedImagesSettingsRequest.to_query req))))) in
  (`POST, uri, [])
let of_http body =
  try
           ...
           BadResponse
             {
               body;
               message =
                 "Could not find well formed GetAllowedImagesSettingsResult."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing GetAllowedImagesSettingsResult - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
