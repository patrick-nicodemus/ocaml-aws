...
let make ~metric_names  ~start_time  ~end_time  ~period  ?(group_by= []) 
  ?(filter_by= [])  ?max_results  ?next_token  ?dry_run  () =
  {
    metric_names;
    start_time;
    end_time;
  ...
  Some
    {
      ...
      group_by =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "GroupBy" xml)
              GroupBySet.parse));
      filter_by =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "FilterBy" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("FilterBy", (CapacityManagerConditionSet.to_query v.filter_by)));
       ...
       Some
         (Aws.Query.Pair ("MetricName", (MetricSet.to_query v.metric_names)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.dry_run
...
let of_json j =
  {
    ...
    period =
      (Integer.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "Period")));
    group_by =
      (GroupBySet.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "GroupBy")));
    filter_by =
