...
let make ?(block_device_mappings= [])  ?ebs_optimized  ?iam_instance_profile 
  ~image_id  ?instance_type  ?kernel_id  ?key_name  ?monitoring 
  ?(network_interfaces= [])  ?placement  ?ramdisk_id  ?(security_group_ids=
  [])  ?subnet_id  ?user_data  () =
  {
       ...
       Some
         ("NetworkInterface",
           (ScheduledInstancesNetworkInterfaceSet.to_json
              v.network_interfaces));
       Aws.Util.option_map v.monitoring
         (fun f -> ("Monitoring", (ScheduledInstancesMonitoring.to_json f)));
       Aws.Util.option_map v.key_name
