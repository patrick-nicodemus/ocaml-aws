...
let make ?group_id  ?vpc_id  ?vpc_owner_id  ?state  ?state_reason 
  ?group_owner_id  () =
  ...
  Some
    {
      ...
      vpc_owner_id =
        (Aws.Util.option_bind (Aws.Xml.member "vpcOwnerId" xml) String.parse);
      state =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.state
         (fun f ->
            Aws.Query.Pair
              ("State", (SecurityGroupVpcAssociationState.to_query f)));
       Aws.Util.option_map v.vpc_owner_id
         (fun f -> Aws.Query.Pair ("VpcOwnerId", (String.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.state
         (fun f -> ("state", (SecurityGroupVpcAssociationState.to_json f)));
       Aws.Util.option_map v.vpc_owner_id
         (fun f -> ("vpcOwnerId", (String.to_json f)));
...
let of_json j =
  {
    ...
    vpc_owner_id =
      (Aws.Util.option_map (Aws.Json.lookup j "vpcOwnerId") String.of_json);
    state =
