...
let make ?dry_run  ?vpc_endpoint_type  ~vpc_id  ?service_name 
  ...
  ?(security_group_ids= [])  ?ip_address_type  ?dns_options  ?client_token 
  ?private_dns_enabled  ?(tag_specifications= [])  ?(subnet_configurations=
  [])  ?service_network_arn  ?resource_configuration_arn  ?service_region  ()
  =
  {
    dry_run;
    vpc_endpoint_type;
