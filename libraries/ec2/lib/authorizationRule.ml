  ...
  Some
    {
      ...
      access_all =
        (Aws.Util.option_bind (Aws.Xml.member "accessAll" xml) Boolean.parse);
      destination_cidr =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.access_all
         (fun f -> Aws.Query.Pair ("AccessAll", (Boolean.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.access_all
         (fun f -> ("accessAll", (Boolean.to_json f)));
       Aws.Util.option_map v.group_id
...
let of_json j =
  {
    ...
    access_all =
      (Aws.Util.option_map (Aws.Json.lookup j "accessAll") Boolean.of_json);
    destination_cidr =
