  ...
  Some
    {
      ...
      all_regions =
        (Aws.Util.option_bind (Aws.Xml.member "AllRegions" xml) Boolean.parse);
      dry_run =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.all_regions
         (fun f -> Aws.Query.Pair ("AllRegions", (Boolean.to_query f)));
       Some
         (Aws.Query.Pair
            ("RegionName", (RegionNameStringList.to_query v.region_names)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.all_regions
         (fun f -> ("AllRegions", (Boolean.to_json f)));
...
let of_json j =
  {
    ...
    all_regions =
      (Aws.Util.option_map (Aws.Json.lookup j "AllRegions") Boolean.of_json);
    dry_run =
