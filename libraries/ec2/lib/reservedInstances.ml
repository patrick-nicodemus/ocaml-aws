...
let make ?currency_code  ?instance_tenancy  ?offering_class  ?offering_type 
  ?(recurring_charges= [])  ?scope  ?(tags= [])  ?availability_zone_id 
  ?reserved_instances_id  ?instance_type  ?availability_zone  ?start  ?end_ 
  ?duration  ?usage_price  ?fixed_price  ?instance_count 
