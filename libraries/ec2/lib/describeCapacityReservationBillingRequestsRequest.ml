...
let make ?(capacity_reservation_ids= [])  ~role  ?next_token  ?max_results 
  ?(filters= [])  ?dry_run  () =
  { capacity_reservation_ids; role; next_token; max_results; filters; dry_run
  }
let parse xml =
  Some
    {
      ...
      role =
        (Aws.Xml.required "Role"
           (Aws.Util.option_bind (Aws.Xml.member "Role" xml) CallerRole.parse));
      next_token =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some (Aws.Query.Pair ("Role", (CallerRole.to_query v.role)));
       Some
         (Aws.Query.Pair
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Some ("Role", (CallerRole.to_json v.role));
       Some
         ("CapacityReservationId",
...
let of_json j =
  {
    ...
    role =
      (CallerRole.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "Role")));
    next_token =
