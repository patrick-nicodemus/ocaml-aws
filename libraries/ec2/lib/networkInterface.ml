...
let make ?association  ?attachment  ?availability_zone 
  ?connection_tracking_configuration  ?description  ?(groups= []) 
  ?interface_type  ?(ipv6_addresses= [])  ?mac_address  ?network_interface_id
   ?outpost_arn  ?owner_id  ?private_dns_name  ?public_dns_name 
  ?public_ip_dns_name_options  ?private_ip_address  ?(private_ip_addresses=
  [])  ?(ipv4_prefixes= [])  ?(ipv6_prefixes= [])  ?requester_id 
  ?requester_managed  ?source_dest_check  ?status  ?subnet_id  ?(tag_set= [])
   ?vpc_id  ?deny_all_igw_traffic  ?ipv6_native  ?ipv6_address  ?operator 
  ?(associated_subnets= [])  ?availability_zone_id  () =
  {
    association;
    attachment;
    availability_zone;
  ...
  Some
    {
      ...
      public_dns_name =
        (Aws.Util.option_bind (Aws.Xml.member "publicDnsName" xml)
           String.parse);
      public_ip_dns_name_options =
        (Aws.Util.option_bind (Aws.Xml.member "publicIpDnsNameOptions" xml)
      ...
      deny_all_igw_traffic =
        (Aws.Util.option_bind (Aws.Xml.member "denyAllIgwTraffic" xml)
           Boolean.parse);
      ipv6_native =
      ...
      associated_subnets =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "associatedSubnetSet" xml)
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("AssociatedSubnetSet",
              (AssociatedSubnetList.to_query v.associated_subnets)));
       ...
       Aws.Util.option_map v.deny_all_igw_traffic
         (fun f -> Aws.Query.Pair ("DenyAllIgwTraffic", (Boolean.to_query f)));
       ...
       Aws.Util.option_map v.requester_id
         (fun f -> Aws.Query.Pair ("RequesterId", (String.to_query f)));
       Some
         (Aws.Query.Pair
            ("Ipv6PrefixSet", (Ipv6PrefixesList.to_query v.ipv6_prefixes)));
       Some
         (Aws.Query.Pair
            ("Ipv4PrefixSet", (Ipv4PrefixesList.to_query v.ipv4_prefixes)));
       Some
         (Aws.Query.Pair
       ...
       Aws.Util.option_map v.public_ip_dns_name_options
         (fun f ->
            Aws.Query.Pair
              ("PublicIpDnsNameOptions", (PublicIpDnsNameOptions.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Some
         ("associatedSubnetSet",
           (AssociatedSubnetList.to_json v.associated_subnets));
       Aws.Util.option_map v.operator
       ...
       Aws.Util.option_map v.deny_all_igw_traffic
         (fun f -> ("denyAllIgwTraffic", (Boolean.to_json f)));
       ...
       Aws.Util.option_map v.requester_id
         (fun f -> ("requesterId", (String.to_json f)));
       ...
       Aws.Util.option_map v.interface_type
         (fun f -> ("interfaceType", (NetworkInterfaceType.to_json f)));
       ...
       Aws.Util.option_map v.connection_tracking_configuration
         (fun f ->
            ("connectionTrackingConfiguration",
              (ConnectionTrackingConfiguration.to_json f)));
       Aws.Util.option_map v.availability_zone
...
let of_json j =
  {
    ...
    public_ip_dns_name_options =
      (Aws.Util.option_map (Aws.Json.lookup j "publicIpDnsNameOptions")
    ...
    requester_id =
      (Aws.Util.option_map (Aws.Json.lookup j "requesterId") String.of_json);
    requester_managed =
    ...
    deny_all_igw_traffic =
      (Aws.Util.option_map (Aws.Json.lookup j "denyAllIgwTraffic")
         Boolean.of_json);
    ipv6_native =
