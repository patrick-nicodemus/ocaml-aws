...
let make ?client_data  ?client_token  ?description  ?disk_container  ?dry_run
   ?encrypted  ?kms_key_id  ?role_name  ?(tag_specifications= [])  () =
  {
    client_data;
    client_token;
    description;
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.disk_container
         (fun f ->
            Aws.Query.Pair
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.disk_container
         (fun f -> ("DiskContainer", (SnapshotDiskContainer.to_json f)));
       Aws.Util.option_map v.description
...
let of_json j =
  {
    ...
    disk_container =
      (Aws.Util.option_map (Aws.Json.lookup j "DiskContainer")
