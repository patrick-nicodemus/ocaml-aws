...
let make ~transit_gateway_route_table_id  ~peering_attachment_id 
  ...
  Some
    {
      ...
      peering_attachment_id =
        (Aws.Xml.required "PeeringAttachmentId"
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("PeeringAttachmentId",
              (String.to_query v.peering_attachment_id)));
       Some
         (Aws.Query.Pair
...
let of_json j =
  {
    ...
    peering_attachment_id =
      (String.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "PeeringAttachmentId")));
    tag_specifications =
