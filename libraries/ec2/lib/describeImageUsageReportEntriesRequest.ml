...
let make ?(image_ids= [])  ?(report_ids= [])  ?next_token  ?(filters= []) 
  ?dry_run  ?max_results  () =
  ...
  Some
    {
      ...
      report_ids =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "ReportId" xml)
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("ReportId",
              (ImageUsageReportIdStringList.to_query v.report_ids)));
       Some
         (Aws.Query.Pair
            ("ImageId",
              (DescribeImageUsageReportsImageIdStringList.to_query
                 v.image_ids)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Some
         ("ImageId",
           (DescribeImageUsageReportsImageIdStringList.to_json v.image_ids))])
let of_json j =
  {
    image_ids =
