...
let parse xml =
  Some
    { elastic_gpu_ids =
        Aws.Util.of_option
          []
          (Aws.Util.option_bind (Aws.Xml.member "ElasticGpuId" xml) ElasticGpuIdSet.parse)
...
let of_json j =
  { elastic_gpu_ids =
      ElasticGpuIdSet.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "ElasticGpuId"))
