  ...
  Some
    {
      ...
      option_values =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "optionValueSet" xml)
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Some
          (Aws.Query.Pair
             ("OptionValueSet",
               (ImageUsageResourceTypeOptionValuesList.to_query
                  v.option_values)));
       Aws.Util.option_map v.option_name
         (fun f -> Aws.Query.Pair ("OptionName", (String.to_query f)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Some
       ...
       Aws.Util.option_map v.option_name
         (fun f -> ("optionName", (String.to_json f)))])
let of_json j =
  {
    option_name =
      (Aws.Util.option_map (Aws.Json.lookup j "optionName") String.of_json);
    option_values =
