...
let make ?ena_srd_specification  ?enable_primary_ipv6 
  ?connection_tracking_specification  ?associate_public_ip_address 
  ?(associated_subnet_ids= [])  ?dry_run  ~network_interface_id  ?description
  ...
  Some
    {
      ...
      associated_subnet_ids =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "AssociatedSubnetId" xml)
              SubnetIdList.parse));
      dry_run =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.attachment
          (fun f ->
             Aws.Query.Pair
               ("Attachment", (NetworkInterfaceAttachmentChanges.to_query f)));
       Some
         (Aws.Query.Pair
            ("SecurityGroupId",
       ...
       Some
         (Aws.Query.Pair
            ("AssociatedSubnetId",
              (SubnetIdList.to_query v.associated_subnet_ids)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Some
         ("AssociatedSubnetId",
           (SubnetIdList.to_json v.associated_subnet_ids));
       Aws.Util.option_map v.associate_public_ip_address
