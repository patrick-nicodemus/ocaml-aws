...
let make ?(filters = []) ?(subnet_ids = []) ?next_token ?max_results ?dry_run () =
  { filters; subnet_ids; next_token; max_results; dry_run }
...
let parse xml =
  Some
    ...
    ; subnet_ids =
        Aws.Util.of_option
          []
          (Aws.Util.option_bind (Aws.Xml.member "SubnetId" xml) SubnetIdStringList.parse)
