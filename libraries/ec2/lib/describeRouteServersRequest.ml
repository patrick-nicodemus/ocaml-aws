...
let make ?(route_server_ids= [])  ?next_token  ?max_results  ?(filters= []) 
  ?dry_run  () =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("RouteServerId",
              (RouteServerIdsList.to_query v.route_server_ids)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.dry_run
       ...
       Some
         ("RouteServerId", (RouteServerIdsList.to_json v.route_server_ids))])
let of_json j =
  {
    route_server_ids =
