...
let make ?(instance_types= [])  ~target_capacity  ?target_capacity_unit_type 
  ?single_availability_zone  ?(region_names= []) 
  ?instance_requirements_with_metadata  ?dry_run  ?max_results  ?next_token 
  () =
  {
    instance_types;
    target_capacity;
  ...
  Some
    {
      ...
      region_names =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "RegionName" xml)
              RegionNames.parse));
      instance_requirements_with_metadata =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.instance_requirements_with_metadata
         (fun f ->
            Aws.Query.Pair
       ...
       Some
         (Aws.Query.Pair
            ("RegionName", (RegionNames.to_query v.region_names)));
       ...
       Some
         (Aws.Query.Pair
            ("InstanceType", (InstanceTypes.to_query v.instance_types)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.next_token
