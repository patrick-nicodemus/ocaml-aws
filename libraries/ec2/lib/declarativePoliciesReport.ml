  ...
  Some
    {
      ...
      status =
        (Aws.Util.option_bind (Aws.Xml.member "status" xml) ReportState.parse);
      tags =
        (Aws.Util.of_option []
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.status
         (fun f -> Aws.Query.Pair ("Status", (ReportState.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.status
         (fun f -> ("status", (ReportState.to_json f)));
       Aws.Util.option_map v.end_time
...
let of_json j =
  {
    ...
    status =
      (Aws.Util.option_map (Aws.Json.lookup j "status") ReportState.of_json);
    tags =
