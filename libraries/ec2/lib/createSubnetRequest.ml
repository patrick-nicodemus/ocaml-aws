...
let make ?(tag_specifications= [])  ?availability_zone  ?availability_zone_id
   ?cidr_block  ?ipv6_cidr_block  ?outpost_arn  ~vpc_id  ?ipv6_native 
  ?ipv4_ipam_pool_id  ?ipv4_netmask_length  ?ipv6_ipam_pool_id 
  ?ipv6_netmask_length  ?dry_run  () =
  {
    tag_specifications;
    availability_zone;
