  ...
  Some
    {
      ...
      ipam_ids =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "IpamId" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Some
          (Aws.Query.Pair ("IpamId", (ValueStringList.to_query v.ipam_ids)));
...
let of_json j =
  {
    ...
    ipam_ids =
      (ValueStringList.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "IpamId")))
  }