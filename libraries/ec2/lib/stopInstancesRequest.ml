  ...
  Some
    {
      ...
      hibernate =
        (Aws.Util.option_bind (Aws.Xml.member "Hibernate" xml) Boolean.parse);
      skip_os_shutdown =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.hibernate
         (fun f -> Aws.Query.Pair ("Hibernate", (Boolean.to_query f)));
       Some
         (Aws.Query.Pair
...
let of_json j =
  {
    ...
    hibernate =
      (Aws.Util.option_map (Aws.Json.lookup j "Hibernate") Boolean.of_json);
    skip_os_shutdown =
