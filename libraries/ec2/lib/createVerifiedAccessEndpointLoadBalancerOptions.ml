...
let make ?protocol  ?port  ?load_balancer_arn  ?(subnet_ids= []) 
  ?(port_ranges= [])  () =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Some
          (Aws.Query.Pair
             ("PortRange",
               (CreateVerifiedAccessEndpointPortRangeList.to_query
                  v.port_ranges)));
       Some
         (Aws.Query.Pair
            ("SubnetId",
              (CreateVerifiedAccessEndpointSubnetIdList.to_query v.subnet_ids)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Some
          ("PortRange",
            (CreateVerifiedAccessEndpointPortRangeList.to_json v.port_ranges));
       Some
         ("SubnetId",
