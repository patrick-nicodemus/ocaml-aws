...
let make ?dry_run  ?next_token  ?max_results  ?(instance_ids= []) 
  ?(group_names= [])  ?(filters= [])  () =
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("GroupName",
              (DescribeInstanceTopologyGroupNameSet.to_query v.group_names)));
       Some
         (Aws.Query.Pair
            ("InstanceId",
              (DescribeInstanceTopologyInstanceIdSet.to_query v.instance_ids)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Some
         ("GroupName",
           (DescribeInstanceTopologyGroupNameSet.to_json v.group_names));
       Some
         ("InstanceId",
           (DescribeInstanceTopologyInstanceIdSet.to_json v.instance_ids));
       Aws.Util.option_map v.max_results
