...
let make ~capacity_reservation_fleet_id  ?total_target_capacity  ?end_date 
  ?dry_run  ?remove_end_date  () =
  {
  ...
  Some
    {
      ...
      remove_end_date =
        (Aws.Util.option_bind (Aws.Xml.member "RemoveEndDate" xml)
           Boolean.parse)
    }
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.remove_end_date
          (fun f -> Aws.Query.Pair ("RemoveEndDate", (Boolean.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.remove_end_date
          (fun f -> ("RemoveEndDate", (Boolean.to_json f)));
       Aws.Util.option_map v.dry_run
       ...
       Some
         ("CapacityReservationFleetId",
           (String.to_json v.capacity_reservation_fleet_id))])
let of_json j =
  {
    capacity_reservation_fleet_id =
      (String.of_json
    ...
    remove_end_date =
      (Aws.Util.option_map (Aws.Json.lookup j "RemoveEndDate")
         Boolean.of_json)
  }