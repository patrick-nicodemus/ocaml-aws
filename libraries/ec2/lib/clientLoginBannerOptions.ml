  ...
  Some
    {
      ...
      banner_text =
        (Aws.Util.option_bind (Aws.Xml.member "BannerText" xml) String.parse)
    }
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.banner_text
          (fun f -> Aws.Query.Pair ("BannerText", (String.to_query f)));
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.banner_text
          (fun f -> ("BannerText", (String.to_json f)));
       Aws.Util.option_map v.enabled
...
let of_json j =
  {
    ...
    banner_text =
      (Aws.Util.option_map (Aws.Json.lookup j "BannerText") String.of_json)
  }