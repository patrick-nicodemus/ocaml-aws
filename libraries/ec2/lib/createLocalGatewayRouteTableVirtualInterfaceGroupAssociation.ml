    ...
    try
      Util.or_error
        (Util.option_bind resp
           CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationResult.parse)
        (let open Error in
           BadResponse
             {
               body;
               message =
                 "Could not find well formed CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationResult."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociationResult - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
