...
let make ?dry_run  ~ipam_resource_discovery_id  ~discovery_region  ?(filters=
  ...
  Some
    {
      ...
      discovery_region =
        (Aws.Xml.required "DiscoveryRegion"
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("DiscoveryRegion", (String.to_query v.discovery_region)));
       Some
         (Aws.Query.Pair
...
let of_json j =
  {
    ...
    discovery_region =
      (String.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "DiscoveryRegion")));
    filters =
      (FilterList.of_json
