...
let make ?pool_id  ?description  ?(pool_address_ranges= []) 
  ?total_address_count  ?total_available_address_count  ?network_border_group
   ?(tags= [])  () =
  {
    pool_id;
    description;
  ...
  Some
    {
      ...
      pool_address_ranges =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "poolAddressRangeSet" xml)
              PublicIpv4PoolRangeSet.parse));
      total_address_count =
        (Aws.Util.option_bind (Aws.Xml.member "totalAddressCount" xml)
           Integer.parse);
      total_available_address_count =
        (Aws.Util.option_bind
           (Aws.Xml.member "totalAvailableAddressCount" xml) Integer.parse);
      network_border_group =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.total_available_address_count
         (fun f ->
            Aws.Query.Pair
              ("TotalAvailableAddressCount", (Integer.to_query f)));
       Aws.Util.option_map v.total_address_count
         (fun f -> Aws.Query.Pair ("TotalAddressCount", (Integer.to_query f)));
       Some
         (Aws.Query.Pair
            ("PoolAddressRangeSet",
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Aws.Util.option_map v.total_available_address_count
         (fun f -> ("totalAvailableAddressCount", (Integer.to_json f)));
       Aws.Util.option_map v.total_address_count
         (fun f -> ("totalAddressCount", (Integer.to_json f)));
       Some
         ("poolAddressRangeSet",
...
let of_json j =
  {
    ...
    total_address_count =
      (Aws.Util.option_map (Aws.Json.lookup j "totalAddressCount")
         Integer.of_json);
    total_available_address_count =
      (Aws.Util.option_map (Aws.Json.lookup j "totalAvailableAddressCount")
         Integer.of_json);
    network_border_group =
