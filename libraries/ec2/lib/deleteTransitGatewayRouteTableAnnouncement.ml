...
let of_http body =
  try
    ...
    let resp =
      Xml.member "DeleteTransitGatewayRouteTableAnnouncementResponse" (snd xml)
    in
    try
      Util.or_error
        (Util.option_bind resp DeleteTransitGatewayRouteTableAnnouncementResult.parse)
        (let open Error in
         BadResponse
           { body
           ; message =
    ...
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing DeleteTransitGatewayRouteTableAnnouncementResult - missing \
