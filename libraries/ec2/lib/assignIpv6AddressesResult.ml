...
let make ?(assigned_ipv6_addresses= [])  ?(assigned_ipv6_prefixes= []) 
  ...
  Some
    {
      assigned_ipv6_addresses =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "assignedIpv6Addresses" xml)
              Ipv6AddressList.parse));
      assigned_ipv6_prefixes =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "assignedIpv6PrefixSet" xml)
              IpPrefixList.parse));
      network_interface_id =
       ...
       Some
         (Aws.Query.Pair
            ("AssignedIpv6PrefixSet",
              (IpPrefixList.to_query v.assigned_ipv6_prefixes)));
       Some
         (Aws.Query.Pair
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       Some
         ("assignedIpv6PrefixSet",
           (IpPrefixList.to_json v.assigned_ipv6_prefixes));
       Some
         ("assignedIpv6Addresses",
