...
let make ?max_results  ?next_token  ?(owner_ids= []) 
  ?(restorable_by_user_ids= [])  ?(snapshot_ids= [])  ?dry_run  ?(filters=
  [])  () =
  {
    max_results;
    next_token;
  ...
  Some
    {
      ...
      restorable_by_user_ids =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "RestorableBy" xml)
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair
            ("SnapshotId", (SnapshotIdStringList.to_query v.snapshot_ids)));
       Some
         (Aws.Query.Pair
            ("RestorableBy",
              (RestorableByStringList.to_query v.restorable_by_user_ids)));
       Some
         (Aws.Query.Pair ("Owner", (OwnerStringList.to_query v.owner_ids)));
