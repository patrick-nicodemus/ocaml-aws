...
let make ?(data_queries= [])  ?start_time  ?end_time  ?max_results 
  ?next_token  ?dry_run  () =
  ...
  Some
    {
      data_queries =
        (Aws.Util.of_option []
           (Aws.Util.option_bind (Aws.Xml.member "DataQuery" xml)
              DataQueries.parse));
      start_time =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       Some
         (Aws.Query.Pair ("DataQuery", (DataQueries.to_query v.data_queries)))])
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [Aws.Util.option_map v.dry_run
...
let of_json j =
  {
    data_queries =
      (DataQueries.of_json
         (Aws.Util.of_option_exn (Aws.Json.lookup j "DataQuery")));
    start_time =
