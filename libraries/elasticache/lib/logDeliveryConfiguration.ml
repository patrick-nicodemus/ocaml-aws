...
let make ?log_type ?destination_type ?destination_details ?log_format ?status ?message ()
    =
  { log_type; destination_type; destination_details; log_format; status; message }
  ...
  Some
    ...
    ; status =
        Aws.Util.option_bind
          ...
          LogDeliveryConfigurationStatus.parse
  ...
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.status (fun f ->
             Aws.Query.Pair ("Status", LogDeliveryConfigurationStatus.to_query f))
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.status (fun f ->
             "Status", LogDeliveryConfigurationStatus.to_json f)
...
let of_json j =
  ...
  ; status =
      Aws.Util.option_map
        ...
        LogDeliveryConfigurationStatus.of_json
