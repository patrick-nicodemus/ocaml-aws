...
type t =
  ...
  ; last_refresh_id : String.t option
  ; last_refresh_failure_reason : String.t option
...
let parse xml =
  Some
    ...
    ; status = Aws.Util.option_bind (Aws.Xml.member "Status" xml) DashboardStatus.parse
    ; widgets =
        Aws.Util.of_option
          []
    ...
    ; last_refresh_id =
        Aws.Util.option_bind (Aws.Xml.member "LastRefreshId" xml) String.parse
    ; last_refresh_failure_reason =
        Aws.Util.option_bind (Aws.Xml.member "LastRefreshFailureReason" xml) String.parse
    ; termination_protection_enabled =
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.last_refresh_failure_reason (fun f ->
             Aws.Query.Pair ("LastRefreshFailureReason", String.to_query f))
       ; Aws.Util.option_map v.last_refresh_id (fun f ->
             Aws.Query.Pair ("LastRefreshId", String.to_query f))
       ...
       ; Aws.Util.option_map v.status (fun f ->
             Aws.Query.Pair ("Status", DashboardStatus.to_query f))
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.last_refresh_failure_reason (fun f ->
             "LastRefreshFailureReason", String.to_json f)
       ; Aws.Util.option_map v.last_refresh_id (fun f ->
             "LastRefreshId", String.to_json f)
       ; Aws.Util.option_map v.updated_timestamp (fun f ->
       ...
       ; Aws.Util.option_map v.status (fun f -> "Status", DashboardStatus.to_json f)
...
let of_json j =
  ...
  ; status = Aws.Util.option_map (Aws.Json.lookup j "Status") DashboardStatus.of_json
  ...
  ; last_refresh_id =
      Aws.Util.option_map (Aws.Json.lookup j "LastRefreshId") String.of_json
  ; last_refresh_failure_reason =
      Aws.Util.option_map (Aws.Json.lookup j "LastRefreshFailureReason") String.of_json
  ; termination_protection_enabled =
      Aws.Util.option_map
