...
type t =
  ...
  ; federation_role_arn : String.t option
  ; partition_keys : PartitionKeyList.t
  }
  ...
  Some
    ...
    ; partition_keys =
        Aws.Util.of_option
          []
          (Aws.Util.option_bind
             (Aws.Xml.member "PartitionKeys" xml)
             PartitionKeyList.parse)
    }
...
let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [ Some
           (Aws.Query.Pair
              ("PartitionKeys.member", PartitionKeyList.to_query v.partition_keys))
       ; Aws.Util.option_map v.federation_role_arn (fun f ->
             Aws.Query.Pair ("FederationRoleArn", String.to_query f))
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [ Some ("PartitionKeys", PartitionKeyList.to_json v.partition_keys)
       ; Aws.Util.option_map v.federation_role_arn (fun f ->
             "FederationRoleArn", String.to_json f)
       ; Aws.Util.option_map v.federation_status (fun f ->
...
let of_json j =
  ...
  ; federation_role_arn =
      Aws.Util.option_map (Aws.Json.lookup j "FederationRoleArn") String.of_json
  ; partition_keys =
      PartitionKeyList.of_json
        (Aws.Util.of_option_exn (Aws.Json.lookup j "PartitionKeys"))
  }
