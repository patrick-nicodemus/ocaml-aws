...
let parse xml =
  Some
    { summaries =
        Aws.Util.of_option
          []
          (Aws.Util.option_bind (Aws.Xml.member "Summaries" xml) ChangeSetSummaries.parse)
...
let of_json j =
  { summaries =
      ChangeSetSummaries.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "Summaries"))
