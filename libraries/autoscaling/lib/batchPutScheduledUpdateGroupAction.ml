      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (BatchPutScheduledUpdateGroupActionType.to_query req)))))
  in
  `POST, uri, [], ""
  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "BatchPutScheduledUpdateGroupActionResult")
    in
    try
      Util.or_error
        (Util.option_bind resp BatchPutScheduledUpdateGroupActionAnswer.parse)
        (let open Error in
         BadResponse
           { body
           ; message =
    ...
    with Xml.RequiredFieldMissing msg ->
      ...
      `Error
        (BadResponse
           { body
           ; message =
               "Error parsing BatchPutScheduledUpdateGroupActionAnswer - missing field \
                in body or children: "
               ^ msg
           })
  with Failure msg ->
