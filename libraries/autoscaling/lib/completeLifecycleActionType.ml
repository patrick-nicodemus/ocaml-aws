  ...
  Some
    ...
    ; lifecycle_action_result =
        Aws.Xml.required
          ...
          (Aws.Util.option_bind (Aws.Xml.member "LifecycleActionResult" xml) String.parse)
...
let of_json j =
  ...
  ; lifecycle_action_result =
      String.of_json (Aws.Util.of_option_exn (Aws.Json.lookup j "LifecycleActionResult"))
