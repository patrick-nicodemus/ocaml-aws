...
let make ~collection_id ?location_name ?next_token ?max_results () =
  { collection_id; location_name; next_token; max_results }
...
let parse xml =
  Some
    ...
    ; location_name = Aws.Util.option_bind (Aws.Xml.member "location" xml) String.parse
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       ...
       ; Aws.Util.option_map v.location_name (fun f -> "location", String.to_json f)
...
let of_json j =
  ...
  ; location_name = Aws.Util.option_map (Aws.Json.lookup j "location") String.of_json
