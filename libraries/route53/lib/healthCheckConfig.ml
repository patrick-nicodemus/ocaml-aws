...
type t =
  ...
  ; routing_control_arn : String.t option
  }
...
let parse xml =
  Some
    ...
    ; routing_control_arn =
        Aws.Util.option_bind (Aws.Xml.member "RoutingControlArn" xml) String.parse
    }

let to_query v =
  Aws.Query.List
    (Aws.Util.list_filter_opt
       [ Aws.Util.option_map v.routing_control_arn (fun f ->
             Aws.Query.Pair ("RoutingControlArn", String.to_query f))
...
let to_json v =
  `Assoc
    (Aws.Util.list_filter_opt
       [ Aws.Util.option_map v.routing_control_arn (fun f ->
             "RoutingControlArn", String.to_json f)
       ; Aws.Util.option_map v.insufficient_data_health_status (fun f ->
...
let of_json j =
  ...
  ; routing_control_arn =
      Aws.Util.option_map (Aws.Json.lookup j "RoutingControlArn") String.of_json
  }
